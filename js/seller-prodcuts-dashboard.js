...
$(function () {
  ...
  var sellerId="s3"
  const filterOrdersBySellerId = (orders, sellerId) => {
    const filteredOrders = [];
      ...
      for (const productId in products) {
        ...
        if (product.sellerId === sellerId) {
          // If the order's sellerId matches the given sellerId, add it to the filtered array
          filteredOrders.push(product);
        }
      }
  ...
  // Example: Filter orders by sellerId "s1"
  const filteredOrders = filterOrdersBySellerId(products, sellerId);
