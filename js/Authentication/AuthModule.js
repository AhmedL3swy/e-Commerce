class AuthModule {
    static activeUser = JSON.parse(localStorage.getItem('activeuser')) || null;
    ...
    static isAdmin(){
        if(AuthModule.activeUser == null || AuthModule.activeUser.role != 'Admin' ){
            const container = document.createElement('div');
            container.innerHTML = this.get404HTML();
            // Replace the body content with the container
            document.body.innerHTML = '';
            document.body.appendChild(container);
            //wait 2sec and then redirect to homepage
